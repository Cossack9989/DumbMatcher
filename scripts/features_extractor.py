...
def initBaseInfo():
    ...
    collection_attributes_tbl["arcv2"] = {
        ...
        "logic": [
            "bset", "bset_s", "bset.c", "bset.f", "bset.p", "bset.ne", "bset.eq", "bset.nc", "bset.nz", "bset.hi", "bset.gt", "bset.ls", "bset.lt",
            "bclr", "bclr_s", "bclr.c", "bclr.f", "bclr.p", "bclr.ne", "bclr.eq", "bclr.nc", "bclr.nz", "bclr.hi", "bclr.gt", "bclr.ls", "bclr.lt",
            "bxor", "bxor_s", "bxor.c", "bxor.f", "bxor.p", "bxor.ne", "bxor.eq", "bxor.nc", "bxor.nz", "bxor.hi", "bxor.gt", "bxor.ls", "bxor.lt",
            "bmsk", "bmsk_s", "bmsk.c", "bmsk.f", "bmsk.p", "bmsk.ne", "bmsk.eq", "bmsk.nc", "bmsk.nz", "bmsk.hi", "bmsk.gt", "bmsk.ls", "bmsk.lt",
            "bmskn", "bmskn_s", "bmskn.c", "bmskn.f", "bmskn.p", "bmskn.ne", "bmskn.eq", "bmskn.nc", "bmskn.nz", "bmskn.hi", "bmskn.gt", "bmskn.ls", "bmskn.lt",
            "neg", "neg_s", "neg.c", "neg.f", "neg.p", "neg.ne", "neg.eq", "neg.nc", "neg.nz", "neg.hi", "neg.gt", "neg.ls", "neg.lt",
            "and", "and_s", "and.c", "and.f", "and.p", "and.ne", "and.eq", "and.nc", "and.nz", "and.hi", "and.gt", "and.ls", "and.lt",
            "or", "or_s", "or.c", "or.f", "or.p", "or.ne", "or.eq", "or.nc", "or.nz", "or.hi", "or.gt", "or.ls", "or.lt",
            "xor", "xor_s", "xor.c", "xor.f", "xor.p", "xor.ne", "xor.eq", "xor.nc", "xor.nz", "xor.hi", "xor.gt", "xor.ls", "xor.lt",
            "bic", "bic_s", "bic.c", "bic.f", "bic.p", "bic.ne", "bic.eq", "bic.nc", "bic.nz", "bic.hi", "bic.gt", "bic.ls", "bic.lt",
        ],
        "shift": [
            "asl", "asl_s", "asl.c", "asl.f", "asl.p", "asl.ne", "asl.eq", "asl.nc", "asl.nz", "asl.hi", "asl.gt", "asl.ls", "asl.lt",
            "asr", "asr_s", "asr.c", "asr.f", "asr.p", "asr.ne", "asr.eq", "asr.nc", "asr.nz", "asr.hi", "asr.gt", "asr.ls", "asr.lt",
            "lsr", "lsr_s", "lsr.c", "lsr.f", "lsr.p", "lsr.ne", "lsr.eq", "lsr.nc", "lsr.nz", "lsr.hi", "lsr.gt", "lsr.ls", "lsr.lt",
            "rol", "rol_s", "rol.c", "rol.f", "rol.p", "rol.ne", "rol.eq", "rol.nc", "rol.nz", "rol.hi", "rol.gt", "rol.ls", "rol.lt",
            "ror", "ror_s", "ror.c", "ror.f", "ror.p", "ror.ne", "ror.eq", "ror.nc", "ror.nz", "ror.hi", "ror.gt", "ror.ls", "ror.lt",
            "rlc", "rlc_s", "rlc.c", "rlc.f", "rlc.p", "rlc.ne", "rlc.eq", "rlc.nc", "rlc.nz", "rlc.hi", "rlc.gt", "rlc.ls", "rlc.lt",
            "rrc", "rrc_s", "rrc.c", "rrc.f", "rrc.p", "rrc.ne", "rrc.eq", "rrc.nc", "rrc.nz", "rrc.hi", "rrc.gt", "rrc.ls", "rrc.lt",
        ],
        "system": [
            "rtie",
            "swi", "swi_s",
...
def get_all_segments(debug=False):
    ...
    for ea in idautils.Segments():
        ...
        if idc.get_segm_attr(ea, idc.SEGATTR_TYPE) == idc.SEG_CODE:
            executable_segments_list.append(segm_name)
    if debug:
        for segname in segments.keys():
            print(segname, hex(segments[segname]['head']), hex(segments[segname]['tail']))
    return segments, executable_segments_list
...
def get_all_names(debug=False):
    ...
    if debug:
        for addr in names.keys():
            print(f"{addr:#x} : {names[addr]}")
    return names
...
def calc_acfg(func, debug=False, decompilable=True):
    ...
    for block in block_list:
        ...
        hash[block.id] = ssdeep.hash(idc.get_bytes(block.start_ea, block.end_ea - block.start_ea))
    ...
    return {"cfg": cfg, "hash": hash, "ea": bb_ea, "attr": bb_attrs}
...
def get_functions(debug=False, decompilable=True):
    ...
    global white_list_function, workdir

    functions = {}

    entries = [entry for entry in idautils.Entries()]           # @return: List of tuples (index, ordinal, ea, name)
    func_info = {}
    [func_info.update({
        idc.get_func_name(func_ea): func_ea
    }) for func_ea in idautils.Functions()]

    for entry in entries:

        if entry[2] not in func_info.values():
            continue
        func_ea = entry[2]
        func_name = entry[3]
        if func_name not in white_list_function:
            continue

        similiar_func_ea = [func_ea]
        related_func_ea = []
        possible_func_names = [
            func_name,
            f".{func_name}", f"_{func_name}", f"_{func_name}_r", f"__{func_name}",
            f"__{func_name}_internal", f"__{func_name}_internal_0"
        ]
        for i in range(10):
            possible_func_names.append(f"{func_name}_{i}")
            possible_func_names.append(f"_{func_name}_{i}")

        for possible_func_name in possible_func_names:
            if possible_func_name in func_info.keys():
                similiar_func_ea.append(func_info[possible_func_name])

        for possible_func_name in func_info.keys():
            if difflib.SequenceMatcher(None, func_name, possible_func_name).ratio() > 0.8 \
                    and func_name in possible_func_name \
                    and possible_func_name not in possible_func_names \
                    and possible_func_name not in white_list_function \
                    and func_info[possible_func_name] != func_ea:
                if possible_func_name.endswith("_0"):
                    new_white_function = possible_func_name[:-2]
                    if new_white_function in func_info.keys() and new_white_function not in white_list_function:
                        white_list_function.append(new_white_function)
                related_func_ea.append(func_info[possible_func_name])

        func_dir = os.path.join(workdir, func_name)
        assert checkDir(func_dir)
        existed_signs = os.listdir(func_dir)

        for _func_ea in similiar_func_ea + related_func_ea:
            func = ida_funcs.get_func(_func_ea)
            ...
            if checkSign(func_sign, existed_signs):
                print(f"{func_sign} hit in {func_dir}")
                continue

            if _func_ea in similiar_func_ea:
                functions[func_name] = {"hash": func_hash, "head": func.start_ea, "tail": func.end_ea, "hcfg": {}}
            else:
                _func_name = idc.get_func_name(_func_ea)
                functions[_func_name] = {"hash": func_hash, "head": func.start_ea, "tail": func.end_ea, "hcfg": {}}

            if decompilable:
                try:
                    body = ida_hexrays.decompile(_func_ea).body
            ...
            if _func_ea in similiar_func_ea:
                ...
                _func_name = idc.get_func_name(_func_ea)
                functions[_func_name]["acfg"] = calc_acfg(func, debug=debug, decompilable=decompilable)
            consts_tbl.clear()
            symbol_tbl.clear()
            calls_vec.clear()
            ...
            if _func_ea in similiar_func_ea:
                json.dump(
                    functions[func_name],
                    open(os.path.join(func_dir, f"{func_sign}.{base_name}.{_func_ea:#x}.json"), "w")
                )
            else:
                _func_name = idc.get_func_name(_func_ea)
                _func_dir = os.path.join(workdir, _func_name)
                assert checkDir(_func_dir)
                _existed_signs = os.listdir(_func_dir)
                if checkSign(func_sign, _existed_signs):
                    print(f"{func_sign} hit in {_func_dir}")
                    continue
                print("Adding", _func_name, func_sign)
                json.dump(
                    functions[_func_name],
                    open(os.path.join(_func_dir, f"{func_sign}.{base_name}.{_func_ea:#x}.json"), "w")
                )
    if debug:
...
def run(debug=False):
    ...
    decompilable = True if (arch in ["metapc", "ARM"]) or (arch in ["mipsl", "mipsb"] and bits is False) else False
...
if "BIN_FEATURE_DB_PATH" in os.environ.keys() \
        and isinstance(os.environ["BIN_FEATURE_DB_PATH"], str) \
        and os.path.exists(os.environ["BIN_FEATURE_DB_PATH"]):
    bin_feature_db_path = os.environ["BIN_FEATURE_DB_PATH"]
else:
    bin_feature_db_path = "/Users/c0ss4ck/Projects/C0ss4ck/BinFeatureDB/V1/"
    print(f"BIN_FEATURE_DB_PATH is not set")

print(f"BIN_FEATURE_DB_PATH is set to {bin_feature_db_path}")

workdir = bin_feature_db_path
...
white_list_function = [
    ...
    'vsprintf', 'vfprintf', 'vsnprintf', 'vsscanf', 'vfscanf', 'strtoull', 'strtoll', 'asprintf',
    'stpncpy', 'stpcpy', 'strchrnul', 'memchr',
]
